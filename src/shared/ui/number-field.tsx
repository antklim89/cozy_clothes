...
interface NumberFieldContextType {
  id: string;
}
...
const inputVariants = cva(
  [
    'border border-input flex items-center justify-center transition-colors select-none disabled:opacity-50 disabled:pointer-events-none w-16 text-center',
...
const buttonVariants = cva(
  [
    'px-6 bg-primary text-primary-foreground hover:bg-primary/80 cursor-pointer focus-visible:outline-hidden inline-flex items-center justify-center border border-input',
    'focus-visible:ring-ring/30  focus-visible:border-ring focus-visible:outline-none focus-visible:ring-[3px]',
...
function NumberField({
  ...
  size = 'md',
  ...props
}: ComponentProps<typeof NumberFieldPrimitive.Root> & VariantProps<typeof inputVariants>) {
  let fieldId = useId();
  if (id) fieldId = id;
  ...
  return (
    <NumberFieldContext.Provider value={{ id: fieldId }}>
      ...
      >
        {children}
        <NumberFieldPrimitive.Group className="flex rounded-md text-foreground shadow-black/5 shadow-xs transition-shadow">
