...
export function ProductsVariantsSelect({
  variants,
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-wrap gap-1">
        {Array.from(colors, ([color, variant]) => {
          const newVariant = (selectedVariant.colorName === color ? selectedVariant : null)
          ...
          return (
            <Button asChild key={color} variant="outline">
              <Link
                className={cn(
                  'uppercase text-md no-underline',
                  {
                ...
                scroll={false}
              >
                <span className="rounded-full size-4 mr-2" style={{ backgroundColor: variant?.[0]?.colorCode }}></span>
                {color}
              </Link>
            </Button>
          );
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-wrap gap-1">
        {Array.from(sizes, ([size, variant]) => {
          const newVariant = (selectedVariant.size === size ? selectedVariant : null)
